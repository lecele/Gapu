# Guapu - Deploy do fix do quiz (TC-RF-007) + texto da pill do cabecalho.
# Gerado pelo Claude em 03/09/2026.
#
# O que vai junto:
#   lib/chat/prompts/modes.ts  -> corrige o bug do quiz (retry + proxima questao
#                                 na mesma mensagem, que dessincronizava o estado)
#   app/page.tsx               -> "Tutor de Enfermagem" vira
#                                 "Tutor para Enfermagem Perioperatoria"
#   lib/chat/references.ts     -> entra so porque o script da VPS confere a
#                                 presenca dele (checagem fixa de 02/09).
#                                 O conteudo dele NAO muda.

$ErrorActionPreference = "Stop"
$projeto = "C:\Users\llece\Documents\DEV\Agentes_na_Saude\Guapu"
$log     = Join-Path $projeto "scratch\deploy-quiz-fix.log"
$chave   = "C:\Users\llece\.ssh\agentesnasa_vps_codex_ed25519"
$destino = "vpsadmin@100.103.17.64"
$porta   = "22022"
$stamp   = "20260903-quizfix"
...
Set-Content -Path $log -Value "Guapu - deploy quiz fix - $(Get-Date)" -Encoding UTF8
...
try {
  ...
  if ($code -ne 0) {
    ...
    Registrar "Volte para a conversa e diga: deploy do quiz falhou"
  } else {
    Registrar "=== DEPLOY CONCLUIDO COM SUCESSO ==="
    Registrar "Volte para a conversa e diga: deploy do quiz pronto"
  }
}
catch {
  Registrar "=== INTERROMPIDO ANTES DE PUBLICAR ==="
  Registrar $_.Exception.Message
  Registrar "Diga na conversa: deploy do quiz falhou"
}
finally {
  Write-Host ""
