# Guapu - Deploy do detector de pedido de resposta pronta de avaliacao.
# Gerado pelo Claude em 04/09/2026. Corrige o TC-GR-004.
#
# Arquivos:
#   lib/chat/exam-request.ts  -> ARQUIVO NOVO: detecta formato/intencao de prova
#   app/api/chat/route.ts     -> liga a checagem antes da recuperacao
...
Set-Content -Path $log -Value "Guapu - deploy exam guard - $(Get-Date)" -Encoding UTF8

$arquivos = @(
  "lib/chat/exam-request.ts",
...
try {
  ...
  if ($code -ne 0) {
    ...
    Registrar "Volte para a conversa e diga: deploy do detector falhou"
  } else {
    Registrar "=== DEPLOY CONCLUIDO COM SUCESSO ==="
    Registrar "Volte para a conversa e diga: deploy do detector pronto"
  }
}
catch {
  Registrar "=== INTERROMPIDO ANTES DE PUBLICAR ==="
  Registrar $_.Exception.Message
  Registrar "Diga na conversa: deploy do detector falhou"
}
finally {
  Write-Host ""
