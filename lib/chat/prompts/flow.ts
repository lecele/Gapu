...
export function buildFlowPrompt({ state, mode, topic, quizQuestion, quizAttempt, studentLevel }: FlowPromptInput): string {
...
- QUESTAO_QUIZ e TENTATIVA_QUIZ vêm do servidor e são a contagem verdadeira: não deduza em qual questão ou em qual tentativa o estudante está a partir do histórico da conversa. TENTATIVA_QUIZ 1 significa que ele ainda não errou esta questão; 2 significa que já errou uma vez.
