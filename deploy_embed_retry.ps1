# Guapu - Deploy do retry no embedding da pergunta do aluno.
# Gerado pelo Claude em 03/09/2026.
#
# Corrige: "Ocorreu uma falha temporaria ao consultar os materiais da
# disciplina" - o embedQuery() nao tinha retry nenhum e um 429 do Gemini
# derrubava a busca na primeira tentativa (3 de 6 pedidos nos testes).
#
# Arquivos:
#   app/api/chat/route.ts    -> retry curto (700ms, 1800ms) em erro transitorio
#   lib/chat/references.ts   -> so por causa da checagem fixa do script da VPS.
#                               O conteudo dele NAO muda.
#
# route.ts e o caminho critico do chat inteiro, entao este script roda o
# "npm run build" local ANTES de publicar e aborta se o build falhar.

$ErrorActionPreference = "Stop"
$projeto = "C:\Users\llece\Documents\DEV\Agentes_na_Saude\Guapu"
$log     = Join-Path $projeto "scratch\deploy-embed-retry.log"
$chave   = "C:\Users\llece\.ssh\agentesnasa_vps_codex_ed25519"
$destino = "vpsadmin@100.103.17.64"
$porta   = "22022"
$stamp   = "20260903-embedretry"
...
Set-Content -Path $log -Value "Guapu - deploy embed retry - $(Get-Date)" -Encoding UTF8
...
try {
  ...
  if ($code -ne 0) {
    ...
    Registrar "Volte para a conversa e diga: deploy do embedding falhou"
  } else {
    Registrar "=== DEPLOY CONCLUIDO COM SUCESSO ==="
    Registrar "Volte para a conversa e diga: deploy do embedding pronto"
  }
}
catch {
  Registrar "=== INTERROMPIDO ANTES DE PUBLICAR ==="
  Registrar $_.Exception.Message
  Registrar "Diga na conversa: deploy do embedding falhou"
}
finally {
  Write-Host ""
