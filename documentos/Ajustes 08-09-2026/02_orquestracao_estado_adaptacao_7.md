...
**Versão:** v1.7.0 · **Escopo:** carregado em toda chamada, depois do Prompt 01. Contém os textos fixos de menu/encerramento, as regras de transição de estado, o controle de verbosidade e a adaptação ao nível do estudante.
...
- **Elementos clicáveis ao final do Resumo (histórico, resolvido por outra via na v1.7.0):** até a v1.5.0, a especificação de saída do Resumo usava texto corrido em uma pergunta única, sem lista com marcadores, especificamente para reduzir a chance de a aplicação transformar aquele trecho em botões. A partir da v1.7.0, o Resumo (como o Quiz e Informações da Disciplina) não gera mais nenhuma pergunta de encerramento própria — ele encerra indo direto para o menu curto (Prompt 03, seção 1; Prompt 02, seção 2), que já é a mensagem fixa e padronizada da aplicação. Isso resolve a preocupação original por outro caminho: não há mais um texto de encerramento gerado pelo modelo, em nenhuma das três modalidades estruturadas, que a aplicação precise interpretar para decidir se vira botão.
